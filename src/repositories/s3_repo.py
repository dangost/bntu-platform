...
class Folders:
    ...
    def content_type(value: str) -> str:
        d = {
            Folders.FILES: "application/octet-stream",
            Folders.IMAGES: "image/jpg"
        }
        return d.get(value, "application/octet-stream")


class S3Repository:
    def __init__(self, access_key: str, secret_key: str, host: str = "127.0.0.1", port: int = 9000):
        self.__client = Minio(
    ...
    def upload_file(self, data: bytes, folder: str, filename: str, size: int = -1) -> str:
    ...
    def get_file(self, path: str) -> str:
        if Folders.FILES not in path:
            raise S3ImageNotFound()
        try:
            url = self.__client.get_presigned_url("GET", "data", path, expires=timedelta(hours=12))
        except S3Error as e:
            raise S3ImageNotFound(str(e))
        return url
